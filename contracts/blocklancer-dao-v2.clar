...
(define-private (finalize-proposal (proposal-id uint))
  (let
    ...
    ;; Early execution: execute when threshold met OR voting period ends
    (if (or
      (>= current-time (get voting-ends-at proposal-data))
      (>= yes-votes required-threshold))
      ;; Threshold met or voting ended, determine result
      (if (>= yes-votes required-threshold)
        ...
        ;; Proposal failed (voting ended without threshold)
        (begin
          (map-set proposals proposal-id
      ...
      ;; Voting still active and threshold not yet met
      (ok true)
    )
  )
)
...
(define-private (execute-dispute-resolution (proposal-id uint))
  (let
    ...
    ;; Call dispute contract to resolve - resolve in favor of client (refund)
    ;; proposal-type-dispute (u0) = resolution-client-wins
    (try! (contract-call? .blocklancer-dispute-v5 dao-resolve-dispute dispute-id proposal-id u1))
...
(define-private (execute-escrow-release (proposal-id uint))
  (let
    ...
    ;; Call escrow contract to release payment to freelancer
    (try! (contract-call? .blocklancer-escrow-v4 dao-release-payment contract-id))
...
(define-private (execute-escrow-refund (proposal-id uint))
  (let
    ...
    (try! (contract-call? .blocklancer-escrow-v4 dao-refund-payment contract-id))
...
;; Admin function to manually add DAO member
;; Use this when membership contract approval doesn't automatically add member
;; Only deployer can call this function
(define-public (admin-add-dao-member (new-member principal))
  (begin
    (try! (assert-not-paused))
