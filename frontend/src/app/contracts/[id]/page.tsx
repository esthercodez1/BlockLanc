...
import { OpenDisputeButton, OpenDisputeModal } from '@/components/dispute';
import { useContractDispute } from '@/hooks/useDisputes';
...
export default function ContractDetailsPage() {
  ...
  const { data: existingDispute } = useContractDispute(contract?.id || null);
  ...
  // Prime block height cache for accurate deadline conversion
  ...
  // Grace period: 144 blocks (~1 day)
  const GRACE_PERIOD_BLOCKS = 144;

  const isMilestoneRefundable = (milestone: Milestone) => {
    if (milestone.status !== MilestoneStatus.PENDING && milestone.status !== MilestoneStatus.SUBMITTED) return false;
    const currentBlock = estimateCurrentBlockHeight();
    return currentBlock > milestone.deadline + GRACE_PERIOD_BLOCKS;
  };

  const handleClaimRefund = async (milestoneId: number) => {
    if (!contract) return;
    try {
      const result = await claimDeadlineRefund(contract.id, milestoneId);
      if (result.success) {
  ...
  const handleAddMilestone = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const deadlineBlockHeight = dateToBlockHeight(new Date(milestoneForm.deadline));
            ...
            {contract.milestones.length === 0 ? (
              ...
              contract.milestones.map((milestone, index) => (
                <div key={milestone.id} className="border border-gray-200 dark:border-gray-700 rounded-lg p-4">
                  ...
                  {/* Fee breakdown shown before approve action */}
                  {userRole === UserRole.CLIENT && milestone.status === MilestoneStatus.SUBMITTED && (
                    <div className="mb-3">
                      <FeeBreakdown grossAmount={milestone.amount} feeRate={150} isPro={isPro} />
                    </div>
                  )}

                  {/* Action buttons based on user role and milestone status */}
                  <div className="flex gap-2">
                    {userRole === UserRole.FREELANCER && (milestone.status === MilestoneStatus.PENDING || milestone.status === MilestoneStatus.REJECTED) && (
                      <button
                    ...
                    {/* Deadline refund button for overdue milestones (client only) */}
                    {userRole === UserRole.CLIENT && isMilestoneRefundable(milestone) && (
                      <button
        ...
        <div className="bg-white dark:bg-gray-900/50 rounded-xl border border-gray-200 dark:border-gray-800 p-6 mt-8">
          ...
          {existingDispute ? (
            <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-4">
              ...
              <button
                onClick={() => router.push(`/disputes/${existingDispute.id}`)}
          ...
          onSuccess={async (disputeId) => {
            ...
            setTimeout(async () => {
              try {
                ...
                const { getContractDispute } = await import('@/lib/disputeContract');
