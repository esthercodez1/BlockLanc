...
export default function StatsCards({ stats, role }: StatsCardsProps) {
  ...
  const clientStats = [
    ...
    },
    {
      ...
      value: formatSTX(stats.pendingPayments),
      icon: (
        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    {
      name: 'Open Disputes',
      value: stats.openDisputes || 0, // FIXED: Handle undefined
      icon: (
        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  const freelancerStats = [
    ...
    {
      name: 'Total Earnings',
      value: formatSTX(stats.totalEarnings || stats.totalEarned), // FIXED: Handle both property names and undefined
      icon: (
        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1" />
        </svg>
    ...
    },
    {
      ...
      value: stats.pendingPayments > 0 ? Math.ceil(stats.pendingPayments / 1000000) : 0,
      icon: (
        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
